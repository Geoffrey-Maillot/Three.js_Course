...
class ThreeJSFire {
  ...
  constructor(particlecount, scale, maxheight, opacitydegression, velocity) {
    ...
    this.Fire = new THREE.PointLight(
      0xff4502,
      0.2 * this.scale,
      2 * this.scale,
    ); // This is the fire parent element
  ...
  animate() {
    // this function is called each engine tick, to update and animate the fire
    for (
      var i = 0;
      i < this.particles.length;
      i++ // for loop , looping on every particle in the array
    ) {
      var particle = this.particles[i];
      if (particle.position.y > this.maxheight) {
        // if the particle reached the maximum allowed height, the particle is reset
      ...
      this.Fire.intensity =
        (Math.random() * (1 - 0.6) + 0.6) * (0.1 * this.scale); // the light os the fire is random every tick, to create a blinking effect
    }
  }
}
...
function animate() {
  ...
  for (var i = 0; i < ParticlesArray.length; i++) {
    ParticlesArray[i].mesh.rotation.y +=
      0.03 * ParticlesArray[i].pivot_rot_speed_y;
  }

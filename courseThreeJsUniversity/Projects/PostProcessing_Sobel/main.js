...
function init()
{
	...
	var sobelbutton = document.getElementById("sobel").onclick = function() {

		if(effect_sobel.enabled)
		{
				effect_sobel.enabled = false;
		}
		else
		{
				effect_sobel.enabled = true;
		}
	...
	effect_sobel = new ShaderPass( SobelOperatorShader );
	effect_sobel.uniforms[ 'resolution' ].value.x = window.innerWidth * (window.devicePixelRatio*6);
	effect_sobel.uniforms[ 'resolution' ].value.y = window.innerHeight * (window.devicePixelRatio*6);
	composer.addPass( effect_sobel );
